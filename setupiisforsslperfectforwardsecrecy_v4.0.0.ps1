...
#
# Modified while waiting on AH to update his script.
#
# After running this script the computer only supports:
# - TLS 1.2
# - TLS 1.3

# Version 4.0.0, see CHANGELOG.txt for changes.
# 
...
# Enable HTTP/3
New-Item 'HKLM:\SYSTEM\CurrentControlSet\services\HTTP\Parameters' -Force | Out-Null
New-ItemProperty -path 'HKLM:\SYSTEM\CurrentControlSet\services\HTTP\Parameters' -name 'EnableHttp3' -value '1' -PropertyType 'DWord' -Force | Out-Null
New-ItemProperty -path 'HKLM:\SYSTEM\CurrentControlSet\services\HTTP\Parameters' -name 'EnableAltSvc' -value '1' -PropertyType 'DWord' -Force | Out-Null
Write-Host 'HTTP/3 has been enabled.'
...
if ([System.Version]$os.Version -lt [System.Version]'10.0') {
  ...
  # this needs to be tidied up - top 3 are tls1.3
  $cipherSuitesOrder = @(
...
$defaultSecureProtocols = @(
  '10240'  # TLS 1.3 and TLS 1.2  0x00002800
)
...
if ([System.Version]$file_version_winhttp_dll -lt [System.Version]"6.1.7601.23375" -or [System.Version]$file_version_webio_dll -lt [System.Version]"6.1.7601.23375") {
  Write-Host 'WinHTTP: Cannot enable TLS 1.2 and 1.3. Please see https://support.microsoft.com/en-us/help/3140245/update-to-enable-tls-1-1-and-tls-1-2-as-a-default-secure-protocols-in for system requirements.'
} else {
  Write-Host 'WinHTTP: Minimum system requirements are met.'
  Write-Host 'WinHTTP: Activate TLS 1.2 and 1.3 only.'
...
Write-Host 'Windows Internet Explorer: Activate TLS 1.2 and 1.3 only.'
